...
const CALENDAR_IDS = {
  main:     "admin@allcleansolutions.ca",
...
function fromMins(mins) { const h=Math.floor(mins/60),m=mins%60,ampm=h>=12?"PM":"AM"; return `${h%12||12}:${String(m).padStart(2,"0")} ${ampm}`; }
function fmtDur(m) { if(m<60) return `${m} min`; if(m%60===0) return `${m/60} hr`; return `${Math.floor(m/60)}h ${m%60}m`; }
function toDateKey(d) { return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`; }
function maxContractors(selected) { let max=0; selected.forEach(id=>{ const c=SERVICES[id]?.contractors||1; if(c>max) max=c; }); return max||2; }

function getDurationsForService(id, customPrice) {
  const svc = SERVICES[id];
  if (svc.durations) return svc.durations;
  return getPriceBracket(customPrice).durations.filter(d => d <= svc.maxMins);
}

function allDurations(selected, windowPrice, pressurePrice, customPrice) {
...
async function pushToGoogleCalendar(booking, customer) {
  await initGapi();
  ...
  const startDateTime=`${dateStr}T${pad(sh)}:${pad(sm)}:00`;
  const endDateTime=`${dateStr}T${pad(eh)}:${pad(em)}:00`;

  const subPrices={clean1:150,clean2:250,clean4:500,accc:1000};
  let totalPrice=0;
  ...
  const results = await Promise.allSettled(
    [...calIds].map(calendarId =>
      window.gapi.client.calendar.events.insert({ calendarId, resource: event })
    )
  );

  const failed = results.filter(r => r.status === "rejected");
  if (failed.length > 0) throw new Error(`${failed.length} calendar(s) failed to update`);
}

// ─── Simulated availability (replace with real GCal freebusy in production) ──
function getCalendarBusy(dateKey, mc) {
...
function getSlotsAvail(dateKey,mc,startMins,durMins) {
...
function getDayAvail(dateKey,mc,dur) {
...
function TimeSlots({ dateKey, selected, durationMins, selectedTime, onSelectTime }) {
  ...
  return (
    <div>
      <div style={{fontSize:11,color:BRAND_DARK,background:BRAND_LIGHT,borderRadius:8,padding:"8px 12px",marginBottom:10}}>
        {mc} contractors available · Jobs can overlap across contractors
      </div>
      {!slots.some(x=>x.av>0)
...
export default function AllCleanBooking() {
  ...
  function toggleService(id){setSelected(prev=>{const n=new Set(prev);n.has(id)?n.delete(id):n.add(id);return n;});setTime(null);setEndTime(null);}
  function handleSelectDate(dt,dk){setDate(dt);setDateKey(dk);setTime(null);setEndTime(null);setDateStr(dt.toLocaleDateString("en-CA",{weekday:"short",month:"short",day:"numeric"}));}
  ...
  async function handleConfirm() {
    ...
    try {
      await pushToGoogleCalendar(
        {selected,date,time,endTime,durationMins,customPrice,windowPrice,pressurePrice},
        customer
      );
      setConfirmed(true);
      setStep(5);
  ...
  function resetAll(){
    ...
    setDurationMins(60);setDate(null);setDateKey(null);setTime(null);setEndTime(null);
  ...
  function Step1(){
    return (
      <div style={S.content}>
        ...
        <div style={{background:"#f5f7fa",borderRadius:8,padding:"10px 14px",marginBottom:14,fontSize:12}}>
      ...
      <div style={S.content}>
        ...
        {fields.map(f=>(
          <div key={f.key} style={{marginBottom:12}}>
            ...
            <input style={S.input} type={f.type} placeholder={f.placeholder} value={customer[f.key]} onChange={e=>setCustomer(p=>({...p,[f.key]:e.target.value}))}/>
          </div>
        ))}
  ...
  function Step4(){
    ...
    const rows=[
      ["Services",names],
      hasWindow&&hasPressure?["Home values",`🪟 $${windowPrice} · 💦 $${pressurePrice}`]:hasWindow?["Home value",`$${windowPrice} (${wBracket.label})`]:hasPressure?["Home value",`$${pressurePrice} (${pBracket.label})`]:null,
